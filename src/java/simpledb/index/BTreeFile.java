...
public class BTreeFile implements DbFile {
	...
	private BTreeLeafPage reverseFindLeafPage(TransactionId tid, Map<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
									   Field f)
			throws DbException, TransactionAbortedException {
		// some code goes here
		if (BTreePageId.LEAF == pid.pgcateg()) {
		...
		BTreeInternalPage internalPage = (BTreeInternalPage) this.getPage(tid, dirtypages, pid, Permissions.READ_ONLY);
		Iterator<BTreeEntry> iterator = internalPage.reverseIterator();
		if (!iterator.hasNext()) {
			throw new DbException("internal page contains >=1 data");
		}
		BTreeEntry entry = iterator.next();
		...
		if (null == f) {
			...
			while (iterator.hasNext() && f.compare(Op.LESS_THAN_OR_EQ, entry.getKey())) {
				entry = iterator.next();
			}
			nextPid = f.compare(Op.LESS_THAN_OR_EQ, entry.getKey())? entry.getLeftChild(): entry.getRightChild();
		}
		return reverseFindLeafPage(tid, dirtypages, nextPid, perm, f);
	}

	BTreeLeafPage reverseFindLeafPage(TransactionId tid, BTreePageId pid,
							   Field f)
			throws DbException, TransactionAbortedException {
		return reverseFindLeafPage(tid, new HashMap<>(), pid, Permissions.READ_ONLY, f);
	}
	...
	public void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
		...
		Tuple[] stealTuples = new Tuple[(sibling.getNumTuples() - page.getNumTuples())/2];
		for (int i=stealTuples.length-1;iterator.hasNext() && i>=0;i--) {
			Tuple stealTuple = iterator.next();
			sibling.deleteTuple(stealTuple);
			page.insertTuple(stealTuple);
		}
		Iterator<Tuple> rightHandPageIterator = (isRightSibling? sibling: page).iterator();
		if (rightHandPageIterator.hasNext()) {
			entry.setKey(rightHandPageIterator.next().getField(keyField));
	...
	public void stealFromLeftInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		int stealNumber = (leftSibling.getNumEntries() - page.getNumEntries())/2;
		Iterator<BTreeEntry> leftSiblingReverseIterator = leftSibling.reverseIterator();
		for (int i=0;i<stealNumber && leftSiblingReverseIterator.hasNext();i++) {
			BTreeEntry stealEntry = leftSiblingReverseIterator.next();
			leftSibling.deleteKeyAndRightChild(stealEntry);

			BTreeEntry newParentEntry = new BTreeEntry(stealEntry.getKey(), parentEntry.getLeftChild(), parentEntry.getRightChild());
			newParentEntry.setRecordId(parentEntry.getRecordId());
			parent.updateEntry(newParentEntry);

			BTreeEntry firstPageEntry = page.iterator().next();
			BTreeEntry newFirstPageEntry = new BTreeEntry(parentEntry.getKey(), stealEntry.getRightChild(), firstPageEntry.getLeftChild());
			page.insertEntry(newFirstPageEntry);
	...
	public void stealFromRightInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		// the corresponding parent entry. Be sure to update the parent
		// pointers of all children in the entries that were moved.
		int stealNumber = (rightSibling.getNumEntries() - page.getNumEntries())/2;
		Iterator<BTreeEntry> rightSiblingReverseIterator = rightSibling.iterator();
		for (int i=0;i<stealNumber && rightSiblingReverseIterator.hasNext();i++) {
			BTreeEntry stealEntry = rightSiblingReverseIterator.next();
			rightSibling.deleteKeyAndLeftChild(stealEntry);

			BTreeEntry newParentEntry = new BTreeEntry(stealEntry.getKey(), parentEntry.getLeftChild(), parentEntry.getRightChild());
			newParentEntry.setRecordId(parentEntry.getRecordId());
			parent.updateEntry(newParentEntry);

			BTreeEntry lastPageEntry = page.reverseIterator().next();
			BTreeEntry newLastPageEntry = new BTreeEntry(parentEntry.getKey(), lastPageEntry.getRightChild(), stealEntry.getLeftChild());
			page.insertEntry(newLastPageEntry);
	...
	public void mergeLeafPages(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		while (rightPageIterator.hasNext()) {
			Tuple mergedTuple = rightPageIterator.next();
		...
		if (null != newRightPageId) {
			BTreeLeafPage newRightLeafPage = (BTreeLeafPage) getPage(tid, dirtypages, newRightPageId, Permissions.READ_WRITE);
			newRightLeafPage.setLeftSiblingId(leftPage.getId());
			dirtypages.put(newRightPageId, newRightLeafPage);
		}
	...
	public void mergeInternalPages(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		// Delete the entry in the parent corresponding to the two pages that are merging -
		// deleteParentEntry() will be useful here
		BTreeEntry mergedEntry = new BTreeEntry(parentEntry.getKey(), null, null);
		Iterator<BTreeEntry> leftPageEntryIterator = leftPage.reverseIterator();
		if (!leftPageEntryIterator.hasNext()) {
			throw new DbException("left page has 0 entry");
		}
		mergedEntry.setLeftChild(leftPageEntryIterator.next().getRightChild());
		Iterator<BTreeEntry> rightPageEntryIterator = rightPage.iterator();
		if (!rightPageEntryIterator.hasNext()) {
			throw new DbException("right page has 0 entry");
		}
		mergedEntry.setRightChild(rightPageEntryIterator.next().getLeftChild());
		leftPage.insertEntry(mergedEntry);

		rightPageEntryIterator = rightPage.iterator();
		while (rightPageEntryIterator.hasNext()) {
			mergedEntry = rightPageEntryIterator.next();
...
class BTreeFileReverseIterator extends AbstractDbFileIterator {
	...
	public void open() throws DbException, TransactionAbortedException {
		BTreeRootPtrPage rootPtr = (BTreeRootPtrPage) Database.getBufferPool().getPage(
				tid, BTreeRootPtrPage.getId(f.getId()), Permissions.READ_ONLY);
		BTreePageId root = rootPtr.getRootId();
		curp = f.reverseFindLeafPage(tid, root, null);
...
class BTreeSearchReverseIterator extends AbstractDbFileIterator {
	...
	public void open() throws DbException, TransactionAbortedException {
		...
		if(ipred.getOp() == Op.EQUALS || ipred.getOp() == Op.LESS_THAN
			...
			curp = f.reverseFindLeafPage(tid, root, ipred.getField());
		}
		else {
			curp = f.reverseFindLeafPage(tid, root, null);
		}
