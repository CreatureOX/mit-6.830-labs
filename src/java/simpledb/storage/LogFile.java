...
public class LogFile {
    ...
    public void rollback(Long tid) throws NoSuchElementException, IOException{
        long firstLogOffset = tidToFirstLogRecord.get(tid);
        // read backwards
        raf.seek(raf.length() - LONG_SIZE);
        long lastLogOffset = raf.readLong();
        while (firstLogOffset < lastLogOffset) {
            raf.seek(lastLogOffset);
            int type = raf.readInt();
            if (UPDATE_RECORD == type) {
            ...
            raf.seek(lastLogOffset - LONG_SIZE);
            lastLogOffset = raf.readLong();
        }
